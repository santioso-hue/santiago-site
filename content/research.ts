...
export const research: ResearchEntry[] = [
  {
    ...
    description:
      "This project aims to develop a pipeline for constructing patient-specific, heterogeneous electrical conductivity head models for transcranial direct current stimulation (tDCS) simulation in Parkinson's disease (PD). Standard tDCS models assign fixed conductivity values to each tissue compartment, ignoring the fact that PD systematically alters brain tissue microstructure. Building on the dataset of Olsson et al. (2025), which provides MR elastography (MRE) stiffness maps and multidimensional diffusion MRI (MD-dMRI) parameter maps for 12 PD patients and 17 healthy controls, we will first establish a conversion model relating MRI-derived tissue parameters, primarily mean diffusivity (MD), to electrical conductivity. The resulting spatially varying conductivity maps will then be incorporated into subject-specific finite element head models constructed using SimNIBS, and the induced E-field distributions will be compared between PD and healthy control models to quantify the dosimetric impact of disease-related tissue changes.",
